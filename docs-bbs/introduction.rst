...
<https://www.ipingthereforeiam.com/bbs/>`_ and the `Telnet BBS Guide
<https://www.telnetbbsguide.com/>`_, cross-referenced against the MUD list maintained for
`muds.modem.xyz <https://muds.modem.xyz/>`_.  SSH port data and BBS metadata (name, sysop,
software, and website) comes from the Telnet BBS Guide's monthly listing.

It is hosted on Github, and all documentation is auto-generated by code.  Suggest a change to
`bbslist.txt <https://github.com/jquast/modem.xyz/blob/master/bbslist.txt>`_ to add or remove or fix
an encoding as a pull request.  Feel free to suggest any other changes or fixes.

Telnet banners are rendered using the fonts described above, and `Hack <https://sourcefoundry.org/hack/>`_
